...
ggsave("exitVelocityPrediction_BasedOnHorizLocPitch.png")
...
ggsave("exitVelocityPrediction_BasedOnVertLocPitch.png")
#both plots show where the highest exit velo is based on x and y location of pitch

vis.gam(gam_Small, type='response', plot.type='contour', main = "Exit Velo Heat Map Prediction on X/Y")
...
#can repeat on other 2 vars as well
...
ggsave("releaseSpeed_ExitVeloByKmeansClusters.png")
...
ggsave("knnModel_PredictExitVelo_ErrorBand.png")
...
ggsave("knnModel_PredictLaunchAngle_ErrorBand.png")
#####knn on direction
...
ggsave("knnModel_PredictDirection_ErrorBand.png")
...
# Find the distribution of each of the 3 vars based on pitch type (say there are 4 pitch types) 
# from k means and using the batting/pitch info, repeatedly generate outcome result 
# (from model in assessment 2) for each var based on each pitch type and then randomly 
# grab a exit velo, LA, and direction and pull the assigned outcome (out, 1b, 2b, 3b, hr) 
# then randomly runs 1000 times, take the avg, and assign that average to be the exit velo,
# LA, and direction, respectively, on test data
